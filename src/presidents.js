...
function getAgeAtInauguration(presidentsArr) {
  ...
  presidentsArr.forEach((president) => {
    president["ageAtInuaguration"] = (president.tookOffice-president.birthYear)
  });
...
function getPresidentsBornAfter(presidentsArr, year) {
  const presidentsBornAfter = presidentsArr.filter((presidents) => presidents.birthYear>year)
...
function countRepublicanPresidents(presidentsArr) {
  ...
  return presidentsArr.reduce((acc,presidents) => {
    console.log(presidents,acc)
    if (presidents.party === "Republican"){
      return acc+1
    }else {
      return acc
    }
...
function sortPresidentsByName(presidentsArr) {
  return presidentsArr.sort((president1, president2) => president1.name.localeCompare(president2.name))
}